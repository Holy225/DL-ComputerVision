...
class FeatureExtractor(nn.Module):
    ...
    def __init__(self, n_parts = 8):
        ...
        self.pre_layers = nn.Sequential(
            ...
            LocalResponseNorm(5, 1e-4, 0.75, 1),
        ...
        # 1x1 conv on output channel
        self.conv_input_feat = nn.Conv2d(self.output_channels, 512, 1)
    ...
    def forward(self, x):
        ...
        out = self.d4(out)
        out = self.e4(out)
        out = self.conv_input_feat(out)

        att_weights = torch.sigmoid(self.conv_att(out))

        linear_features = []
        for i in range(self.n_parts):
            masked_feature = out * torch.unsqueeze(att_weights[:, i], 1)
            pooled_feature = F.avg_pool2d(masked_feature, masked_feature.size()[2:4])
            linear_features.append(getattr(self, 'linear_feature{}'.format(i+1))(pooled_feature.view(pooled_feature.size(0), -1)))

        concat_features = torch.cat(linear_features, 1)
