...
def tlwhs_to_tlbrs(tlwhs):
    """
    Convert tlwh format bounding boxes to tlbr
    :param tlwhs: list of tlwh bounding boxes
    :return: List of tlbr bounding boxes
    """
    tlbrs = np.copy(tlwhs)
    if len(tlbrs) == 0:
        return tlbrs
...
def get_color(idx):
    ...
    :param idx: Bounding box index number
...
def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None):
    """
    Draw tracking detections
    :param image: cv2 image
    :param tlwhs: List of tlwh bounding boxes
    :param obj_ids: List of object ids
    :param scores: List of scores
    :return: cv2 image with plotted image
    """
    im = np.copy(image)
...
def plot_trajectory(image, tlwhs, track_ids):
    ...
    :param image: cv2 image
    :param tlwhs: list of tlwh bounding boxes
    :param track_ids: list of bounding boxes tracked ids
...
def plot_detections(image, tlbrs, scores=None, color=(255, 0, 0), ids=None):
    """
    :param image: cv2 image
    :param tlbrs: list of tlbr bounding boxes
    :param scores: Detection scores
    :param color: Detection rectangle color
    :param ids: List of detection ids
    :return: cv2 image with plotted detections
    """
    im = np.copy(image)
    text_scale = max(1, image.shape[1] / 800.)
